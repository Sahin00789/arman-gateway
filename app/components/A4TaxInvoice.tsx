...
import { CheckCircle2, ShieldCheck, Download, RefreshCw, Printer } from "lucide-react";
...
export default function A4TaxInvoice({
  ...
  return (
    <div className="w-full flex flex-col items-center space-y-6">
      ...
      <div className="w-full max-w-[210mm] flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl bg-slate-800 border border-slate-700 text-white print:hidden">
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-300">
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
          Single A4 Page Format • Ready for Print / PDF Export
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => window.print()}
            className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs flex items-center gap-2 transition-all shadow-md"
          >
            <Printer className="w-4 h-4" /> Print / Save PDF
          </button>
          <button
            onClick={onReset}
            className="px-4 py-2 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-200 font-semibold text-xs flex items-center gap-1.5 transition-all"
          >
      ...
      {/* 
        Single A4 Sheet Printable Container
        Standard A4: 210mm width x 297mm height
      */}
      <div 
        id="printable-invoice"
        className="w-full max-w-[210mm] min-h-[297mm] bg-white text-slate-900 p-8 sm:p-10 rounded-2xl shadow-2xl border border-slate-200 print:shadow-none print:border-none print:p-6 print:m-0 print:max-w-none print:w-full print:min-h-0 print:rounded-none flex flex-col justify-between font-sans text-xs relative overflow-hidden"
      >
        
        {/* Print Only CSS Override to ensure 1 single page print without headers/footers */}
        <style jsx global>{`
          @media print {
            body {
        ...
        <div className="space-y-6">
          ...
          {/* Header Row: Company Brand + Tax Invoice Title */}
          <div className="flex justify-between items-start border-b-2 border-slate-900 pb-5">
            
            {/* Left Brand */}
            <div className="flex items-center gap-3">
              <div className="relative h-12 w-12 flex items-center justify-center rounded-xl bg-slate-100 border border-slate-300">
                <Image 
                  src="/Applogo.png" 
          ...
          <div className="grid grid-cols-2 gap-6 p-4 rounded-xl bg-slate-50 border border-slate-200">
            ...
            <div className="space-y-1">
              ...
              <div className="text-[11px] text-slate-700 font-mono">MCC Code: <strong>7372</strong> (Software Services & Web Design)</div>
              <div className="text-[11px] text-slate-700">Support: sahin401099@gmail.com | +91 95634 01099</div>
            </div>

            {/* Right: Customer / Billed To Details */}
          ...
          {/* Tax Calculation & Total Summary Grid */}
          <div className="grid grid-cols-12 gap-6 pt-2">
            
            {/* Left Column: Amount in Words & Payment Verification */}
            ...
            {/* Right Column: Tax Breakdown Table */}
        ...
        {/* Bottom Authorized Signature & Legal Footer (Always at bottom of A4) */}
